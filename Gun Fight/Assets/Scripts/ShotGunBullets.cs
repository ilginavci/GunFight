...
public class ShotGunBullets : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (gameObject.CompareTag("PlayerBullet"))
        {
            ...
            if(this.gameObject.name.Equals ("SGBullet1(Clone)"))
            {
                bulletRB.AddForce(transform.up * power/10);
            }
            else if (this.gameObject.name.Equals("SGBullet2(Clone)"))
            {
                bulletRB.AddForce(Vector2.down * power/10);
            }
            BulletDeviation();
        }
        else
